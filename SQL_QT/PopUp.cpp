...
void PopUp::LaunchConnexion(){
    ...
    //CONNEXION DB
    ui->EtatDBL->setStyleSheet("border: 1px solid black;"); //Ajout d'un cadre au rectangle d'erreur
    if(db.open()) {
        ...
        ui->EtatDBL->setStyleSheet("background-color:red");
    ...
    if (EtatConnexion==1){
    ui->EtatDBL->setStyleSheet("background-color:orange");
